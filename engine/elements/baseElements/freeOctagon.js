...
export function renderFreeOctagon(params = {}, position = {}, context = {}) {
	...
	const points = buildPoints([p.side1, p.side2, p.side3, p.side4, p.side5, p.side6, p.side7, p.side8], p.rotation, baseRadius, scale);

	return `<polygon points="${points}" fill="${fill}" stroke="${stroke}" stroke-width="${strokeWidth}" />`;
}

export const freeOctagonElement = {
	id: "free_octagon",
	geometry: { type: "rect" },
	defaultParams: { ...DEFAULTS },
	render: renderFreeOctagon,
};
